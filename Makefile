...
_update_version:
	...
	@CURRENT_VERSION=$$(grep "MARKETING_VERSION = " iAgent.xcodeproj/project.pbxproj | head -1 | sed 's/.*MARKETING_VERSION = \([0-9.]*\).*/\1/'); \
	...
	sed -i '' "s/MARKETING_VERSION = [0-9.]*/MARKETING_VERSION = $$NEW_VERSION/g" iAgent.xcodeproj/project.pbxproj; \
	sed -i '' "s/CURRENT_PROJECT_VERSION = [0-9]*/CURRENT_PROJECT_VERSION = 1/g" iAgent.xcodeproj/project.pbxproj; \
	echo "$(GREEN)project.pbxproj 版本已更新$(NC)"; \
	if grep -q "github.com/xdfnet/iAgent/releases" README.md 2>/dev/null; then \
		echo "$(YELLOW)更新 README.md release URL...$(NC)"; \
		sed -i "" "s|github.com/xdfnet/iAgent/releases/tag/[^)]*|github.com/xdfnet/iAgent/releases/tag/v$$NEW_VERSION|g" README.md; \
...
push: _require_msg _update_version install package
	...
	@VERSION=$$(grep "MARKETING_VERSION = " iAgent.xcodeproj/project.pbxproj | head -1 | sed 's/.*MARKETING_VERSION = \([0-9.]*\).*/\1/'); \
	ZIP_PATH=$$(find $(PACKAGE_DIR) -name "$(PROJECT_NAME)-$$VERSION-*.zip" -type f | head -1); \
	gh release create "v$$VERSION" --title "iAgent v$$VERSION" --notes "$(MSG)"; \
	if [ -n "$$ZIP_PATH" ]; then \
		gh release upload "v$$VERSION" "$$ZIP_PATH"; \
		echo "$(GREEN)已上传: $$ZIP_PATH$(NC)"; \
	fi; \
	echo "$(GREEN)Release 创建完成: https://github.com/xdfnet/iAgent/releases/tag/v$$VERSION$(NC)"
