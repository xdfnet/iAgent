...
3. `AgentService.execute(...)` 调 `claude` CLI
...
- 兼容单条 JSON 对象和 JSONL（逐行 JSON 事件）
- 命中以下任一条件视为结果消息：
  - `type == "result"`
  - `subtype == "success"`
- 回复内容：取最后一个可用的 `result`
- 会话 ID：取最新的 `session_id`
...
- 会话续接核心是 Claude Code 返回的 `session_id`，必须优先用结构化 JSON 解析，不建议用字符串匹配。
...
- 当前只有 Claude Code 一条执行链，输出解析应继续保持“先判结构再取内容”，避免上游格式变化导致静默失败。
